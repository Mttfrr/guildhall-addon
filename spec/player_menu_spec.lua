...
describe("ui.BuildPlayerMenuItems", function()
    ...
    it("returns the canonical 4 items when no characterIds lookup hits", function()
        ...
        assert.are.equal("Inspect",   items[3].text)
        assert.are.equal("Copy name", items[4].text)
    end)
    ...
    it("appends 'Copy profile link' when characterIds[short] resolves", function()
        ...
        assert.are.equal("Copy profile link", items[5].text)
    end)
    ...
    it("Inspect calls NotifyInspect with the short name", function()
        local inspectedTarget
        _G.NotifyInspect = function(name) inspectedTarget = name end
        local items = ui.BuildPlayerMenuItems("Foo-Realm", "WARRIOR")
        items[3].func()   -- Inspect
        assert.are.equal("Foo", inspectedTarget)
        _G.NotifyInspect = nil
    end)
...
describe("ui.OpenPlayerContextMenu", function()
    ...
    it("builds a menu with the short name as title + every action below", function()
        ...
        -- 1 title + 4 player actions = 5 children at the root.
        assert.are.equal(5, #root._children,
            "menu must render 1 title + 4 action buttons")
